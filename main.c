...
static inline void led_on(uint8_t pin){ GPIOA->BSRR = BIT(pin); }
static inline void led_toggle(uint8_t pin){ GPIOA->ODR ^= BIT(pin); }
...
int main(void){
    ...
    while(1){
        ...
        if (falling_edge(&pb4_last, now4, DEBOUNCE_DELAY)){
            if (!cd_active){
                pb4_zero_mode ^= 1;
                if (pb4_zero_mode){ seg_show(0); leds_all_off(); }
                else seg_show(seg_digit_live);
            }
        }
        pb4_last = now4;
